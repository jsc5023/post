...
class CustomAuthenticationFilter(
    ...
    private fun authTokenFromRequest(): AuthToken? {

        if (isAuthorizationHeader()) {

            val authorizationHeader = rq.getHeader("Authorization")
            val authToken = authorizationHeader?.removePrefix("Bearer ")
            val tokenBits = authToken?.split(" ".toRegex(), limit = 2)
    ...
    override fun doFilterInternal(
        ...
        if (listOf("/api/v1/members/login", "/api/v1/members/join", "/api/*/members/logout").contains(url)) {
