...
class Ut {
    object file {
        ...
        fun downloadByHttp(url: String, dirPath: String, uniqueFilename: Boolean): String {
            ...
            filename =
                if (uniqueFilename) UUID.randomUUID().toString() + ORIGINAL_FILE_NAME_SEPARATOR + filename
            else
                filename
        ...
        fun getOriginalFileName(filePath: String?): String {
            ...
            return if (originalFileName.contains(ORIGINAL_FILE_NAME_SEPARATOR))
                originalFileName.substring(originalFileName.indexOf(ORIGINAL_FILE_NAME_SEPARATOR) +
        ...
        private fun getExtensionFromResponse(response: HttpResponse<*>): String {
            return response.headers()
                ...
                .map { contentType: String -> MIME_TYPE_MAP[contentType] ?: "tmp" }
                .orElse("tmp")
        }
    ...
    object json {
        private val objectMapper = AppConfig.getObjectMapper()
