...
import com.example.upload.global.app.AppConfig.Companion.getSiteFrontUrl
...
class SecurityConfig(
    ...
    fun securityFilterChain(http: HttpSecurity): SecurityFilterChain {
        http {
            ...
            addFilterAfter<UsernamePasswordAuthenticationFilter>(customAuthenticationFilter)

            exceptionHandling {
                authenticationEntryPoint = AuthenticationEntryPoint { request, response, authException ->
                    response.contentType = "application/json;charset=UTF-8"
                    response.status = 401
                    response.writer.write(Ut.json.toString(RsData<Empty>("401-1", "잘못된 인증키입니다.")))
                }

                accessDeniedHandler = AccessDeniedHandler {request, response, authException ->
                    response.contentType = "application/json;charset=UTF-8"
                    response.status = 403
                    response.writer.write(Ut.json.toString(RsData<Empty>("403-1", "접근 권한이 없습니다.")))
                }
            }
        }
    ...
    fun corsConfigurationSource(): UrlBasedCorsConfigurationSource {
        val configuration = CorsConfiguration().apply {
            allowedOrigins = listOf("https://cdpn.io", getSiteFrontUrl())
